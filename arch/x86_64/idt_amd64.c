...
static struct idt_gate64 idt[256];
...
static void idt_set64(uint8_t vec, uintptr_t handler)
{
    uint64_t h = (uint64_t)handler;

    idt[vec].offset_lo = (uint16_t)(h & 0xFFFFu);
    idt[vec].sel = 0x08;
    idt[vec].ist = 0;
    idt[vec].flags = 0x8Eu;
    idt[vec].offset_mid = (uint16_t)((h >> 16) & 0xFFFFu);
    idt[vec].offset_hi = (uint32_t)(h >> 32);
    idt[vec].reserved_zero = 0;
}
...
void idt_init(void)
{
    ...
    for (size_t i = 0; i < 256; ++i) {
        ...
        idt[i].offset_mid = 0;
        idt[i].offset_hi = 0;
        idt[i].reserved_zero = 0;
    }

    for (size_t i = 0; i < 32; ++i)
        idt_set64((uint8_t)i, (uintptr_t)stubs[i]);

    {
        ...
        for (size_t i = 0; i < 16; ++i)
            idt_set64((uint8_t)(32 + i), (uintptr_t)irq_stubs[i]);
    }

    idt_set64(64u, (uintptr_t)irq_64);

    {
        struct idtr_amd64 idtr;
