...
extern void cpu_switch(uint64_t *old_sp_save, uint64_t new_sp);
...
/*
 * Fake frame as if cpu_switch had pushed r15..rbp (low addr → high):
 * pop r15..rbp then ret to thread_trampoline.
 */
static void setup_initial_stack(struct thread *t)
{
    ...
    sp[4] = 0;
    sp[5] = 0;
    sp[6] = (uint64_t)(uintptr_t)thread_trampoline;
...
{
    serial_puts("[VerveOS] coop threads: spawning A,B (LAPIC vec 64 → sched_timer_tick)\r\n");
